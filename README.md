...
```powershell
$s=$null; foreach($u in @("https://raw.githubusercontent.com/luisafricano/Olimpo/main/install.ps1","https://brexum.ar/repos/Olimpo/install.ps1")){for($i=1;$i-le 3;$i++){try{$r=Invoke-WebRequest -Uri $u -UseBasicParsing;$s=if($r.Content -is [byte[]]){[System.Text.Encoding]::UTF8.GetString($r.Content)}else{$r.Content};break}catch{Start-Sleep (2*$i)}};if($s){break}}; if(-not $s){throw "No se pudo descargar install.ps1 desde ninguna fuente"}; iex $s
```
...
```powershell
$s=$null; foreach($u in @("https://raw.githubusercontent.com/luisafricano/Olimpo/main/uninstall.ps1","https://brexum.ar/repos/Olimpo/uninstall.ps1")){for($i=1;$i-le 3;$i++){try{$r=Invoke-WebRequest -Uri $u -UseBasicParsing;$s=if($r.Content -is [byte[]]){[System.Text.Encoding]::UTF8.GetString($r.Content)}else{$r.Content};break}catch{Start-Sleep (2*$i)}};if($s){break}}; if(-not $s){throw "No se pudo descargar uninstall.ps1 desde ninguna fuente"}; iex $s
```
