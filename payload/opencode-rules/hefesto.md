...
- Corré `hermes --save` (guarda solo lo nuevo desde el último guardado, no duplica) al terminar una fase del plan, al recibir cualquier error, al recibir una instrucción de Claude (`[CLAUDE]`), y al recibir una instrucción de Zeus. Es aparte de exportar al cerrar la sesión — esto es ir dejando rastro mientras trabajás.
